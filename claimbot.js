...
async function fetchBuildingsInGame(account) {
	return await fetchTable("farmersworld", "buildings", "farmersworld", account, 2);
}
...
async function fetchSeed(account, index = 0) {
	...
	try {
		const endpoint = Configs.atomicEndpoints[index];
		const response = await axios.get(`${endpoint}/atomicassets/v1/assets`, {
			params: { owner: account, collection_name: "farmersworld", schema_name: "plants", page: 1, limit: 10 },
			timeout: 5e3,
		});

		return response.data.data;
	} catch (error) {
		return await fetchSeed(account, index + 1);
	}
}

async function fetchBuilding(account, index = 0) {
	if (index >= Configs.atomicEndpoints.length) {
		return [];
	}
	...
	try {
		const endpoint = Configs.atomicEndpoints[index];
		const response = await axios.get(`${endpoint}/atomicassets/v1/assets`, {
			params: { owner: account, collection_name: "farmersworld", schema_name: "farmbuilding", page: 1, limit: 10 },
			timeout: 5e3,
		});

		return response.data.data;
	} catch (error) {
		return await fetchBuilding(account, index + 1);
	}
}
...
function makeClaimBuildAction(account, buildingId) {
	return {
		account: "farmersworld",
		name: "bldclaim",
		authorization: [{ actor: account, permission: "active" }],
		data: { asset_id: buildingId, owner: account },
	};
}
...
function makeWearSeedAction(account, seedId) {
	return {
		account: "atomicassets",
		name: "transfer",
		authorization: [{ actor: account, permission: "active" }],
		data: { asset_ids: [seedId], from: account, memo: `stake`, to: "farmersworld" },
	};
}

function makeStakeBuildingAction(account, seedId) {
	return {
		account: "atomicassets",
		name: "transfer",
		authorization: [{ actor: account, permission: "active" }],
...
function registerAccountAction(account) {
	return {
		account: "farmersworld",
		name: "newuser",
		authorization: [{ actor: account, permission: "active" }],
		data: { owner: account, referral_partner: ""},
	};
}
...
async function wearBuildings(account, privKey) {
	...
	console.log(`Fetching building from account ${cyan(account)}`);
	const buildings = await fetchBuilding(account);
	console.log(`Found ${yellow(buildings.length)} Building`);

	if (buildings.length) {

		console.log("Wear Building");
		...
		for (let i = 0; i < buildings.length; i++) {
			...
			const actions = [makeStakeBuildingAction(account, building.asset_id)];
...
async function claimBuildings(account, privKey) {
	shuffleEndpoints();
	...
	logTask(`Claiming Building`);
	console.log(`Fetching buildings for account ${cyan(account)}`);
	const buildings = await fetchBuildingsInGame(account);

	const claimables = buildings.filter(({ next_availability }) => {
		const next = new Date(next_availability * 1e3);
		return next.getTime() < Date.now();
	});

	console.log(`Found ${yellow(buildings.length)} Building / ${yellow(claimables.length)} Building ready to build`);
...
async function registerAccount(account, privKey) {
	...
	logTask(`Registering Account`);
	console.log(`Registering account ${cyan(account)}`);
	const delay = _.round(_.random(delayMin, delayMax, true), 2);
	const actions = [registerAccountAction(account)];
...
async function runAccounts(accounts) {
	for (let i = 0; i < accounts.length; i++) {
		...
		if (!accountInfo) {
			console.log(`${red("Error")} Account ${cyan(account)} not found`);
			await registerAccount(account, privKey);
		}else{
			console.log(`Account ${cyan(account)} found`);
		}
